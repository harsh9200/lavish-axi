...
3. The artifact route reads the HTML from disk and runs `injectLavishSdk` (`src/html-transform.js`) to append `<script src="/sdk.js?key=...">`. The SDK script is generated by `createSdkJs` in `src/server.js` and runs inside the sandboxed iframe.
...
- The SDK injected into artifacts is a single template-literal string in `createSdkJs`. It executes inside an iframe sandboxed with `allow-scripts allow-forms allow-popups allow-downloads` (no `allow-same-origin`), so it cannot read the chrome's DOM - communication is `postMessage` only.
