...
- **Review rounds** - Lavish snapshots each distinct HTML change under the local state directory while keeping one stable board URL. The chrome's Round picker opens earlier snapshots read-only and shows a visible-text diff against the preceding round. `lavish-axi snapshot <html-file> --label "..."` saves or labels an explicit milestone; identical HTML is deduplicated.
...
- **Agent presence** - The browser shows when no agent is listening and keeps Send usable while the agent is working; messages sent during either listener gap stay durably queued for the next poll.
  Exactly one poll may attach per board. A duplicate poll is refused immediately without adding EventEmitter listeners or disturbing the attached poll, and recovered gap feedback carries a warning in CLI output.
...
| `lavish-axi snapshot <html-file>` | Save a deduplicated review round behind the artifact's existing stable board URL; use `--label` to name a deliberate milestone.                                                                                                                                                                |
...
| `lavish-axi snapshot`    | `--label "..."`       | Name the saved review round; when the HTML matches the latest round, the label updates without creating a duplicate.                                                                                                                |
